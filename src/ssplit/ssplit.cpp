...
// Load a prefix file
void SentenceSplitter::load(std::string const& fname) {
  using namespace std;
...
SentenceSplitter::SentenceSplitter(std::string const& prefix_file) : SentenceSplitter() {
  if(prefix_file.size()) {
    load(prefix_file);
  }
}
...
std::ostream& single_line(std::ostream& out,    // destination stream
...
std::string& single_line(std::string& snt,     // destination stream
                         string_view span,     // text span to be printed in a single line
                         string_view end,      // stuff to put at end of line
                         bool validate_utf) {  // do we need to validate UTF8?
  static Regex P("^\\s*(.*)\\R+\\s*", PCRE2_UTF);
...
int SentenceSplitter::get_prefix_class(string_view piece) const {
...
string_view SentenceSplitter::operator()(string_view* rest) const {
  ...
  while((success = Chunker_RE.consume(rest, &Chunker_M, PCRE2_NO_UTF_CHECK)) > 0) {
    ...
    */
    ...
    if(whitespace_after.size() == 0) {
      ...
      if(punct == "..." && (punct.data() - whole_match.data() > 1)  // not at the beginning
         && tail == "]" && *(punct.data() - 1) == '[') {
        continue;
      }
    }
...
// exceed the size that a string_view can store (apparently int32_t).
// @TODO: verify: this dates back to working with pcrecpp::StringPiece.
string_view readParagraph(const char** start, const char* const stop) {
  string_view par;
  if(*start == stop) {  // no more data
    return par;
  }
...
SentenceStream::SentenceStream(string_view text,
                               SentenceSplitter const& splitter,
                               splitmode mode,
                               bool verify_utf8)
    : SentenceStream(text.data(), text.size(), splitter, mode, verify_utf8) {}

SentenceStream::SentenceStream(char const* data,
                               size_t datasize,
                               SentenceSplitter const& splitter,
                               splitmode mode,
                               bool verify_utf8)
...
bool SentenceStream::operator>>(string_view& snt) {
